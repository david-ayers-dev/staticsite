...
def generate_page(from_path, template_path, dest_path):
    ...
    mrkdwn = markdown_to_html_node(from_content)
    new_html = mrkdwn.to_html()
    title = extract_title(from_content)
    newtitle = template_content.replace("{{ Title }}", title)
    final_temp = newtitle.replace("{{ Content }}", new_html)
    path = os.path.dirname(dest_path)
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=False)
    with open(dest_path, "w") as new_file:
        new_file.write(final_temp)